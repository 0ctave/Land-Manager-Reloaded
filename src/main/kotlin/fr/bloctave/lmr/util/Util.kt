...
object Util {
	...
	fun <T> createListMessage(
		...
		view.list.forEach { text.append("\n").append(entryToText(it)) }

		// Don't need to add the arrows when sending back to the server console
		if (senderIsPlayer)
			createPageArrows(page, maxPage, arrowsCommandToRun)?.let { text.append("\n").append(it) }
		return text
	}

	private fun createPageTitle(titleLangKey: String, page: Int, maxPage: Int): IFormattableTextComponent =
		StringTextComponent("============= ").withStyle(TextFormatting.YELLOW)
			.append(TranslationTextComponent(titleLangKey, page + 1, maxPage + 1).withStyle(TextFormatting.GOLD))
	...
	private fun createArrow(left: Boolean, page: Int, commandToRun: (Int) -> String): IFormattableTextComponent {
		...
		return StringTextComponent(if (left) arrowLeft else arrowRight).apply {
			withStyle(TextFormatting.BOLD, TextFormatting.YELLOW)
			style.withHoverEvent(
				HoverEvent(HoverEvent.Action.SHOW_TEXT, TranslationTextComponent("lmr.command.page", nextPage)))
			style.withClickEvent(ClickEvent(ClickEvent.Action.RUN_COMMAND, commandToRun(nextPage)))
		}
	}

	private fun createBlank(): IFormattableTextComponent =
		StringTextComponent(arrowBlank).withStyle(TextFormatting.GOLD)
