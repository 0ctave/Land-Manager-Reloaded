...
interface AreasCapability : INBTSerializable<CompoundNBT> {
	...
	fun getIntersectingAreas(aabb: AxisAlignedBB): List<Area>
	...
	fun getSmallestArea(areas: Set<Area>): Area?

	fun contains(area: Area): Boolean
	fun smallestIntersectingArea(area: Area): Area?
	fun smallestIncludingArea(area: Area): Area?
	fun validArea(area: Area): Boolean

	fun validAreaIntersections(area: Area): Boolean

	fun validAreaSize(area: Area): Boolean
