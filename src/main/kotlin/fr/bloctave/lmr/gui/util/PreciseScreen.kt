...
open class PreciseScreen(
    ...
    val textureResLoc = ResourceLocation(LandManager.MOD_ID, "textures/gui/$imageName.png")
    var guiLeft = 0
    var guiTop = 0

    var message: Pair<TranslationTextComponent, Int>? = null
    ...
    override fun render(matrixStack: MatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        buttons.firstOrNull { it is LMButton && it.isHovered && it.tooltip.isNotEmpty() }?.let { button ->
            renderToolTip(
                matrixStack,
                (button as LMButton).tooltip.flatMap { font.split(it, 100) },
                mouseX,
                mouseY,
                font
            )
    ...
    fun drawString(
        ...
        if (shadow)
            font.drawShadow(matrixStack, text, x.toFloat(), y.toFloat(), colour)
        else
            font.draw(matrixStack, text, x.toFloat(), y.toFloat(), colour)
    }
    ...
    fun drawLangString(
        ...
        colour: Int = TEXT_COLOUR_TITLE,
        shadow: Boolean = false
    ): Unit = drawString(matrixStack, I18n.get(key), x, y, colour, shadow)
...
//			textToDraw = font.trimStringToWidth(text, maxWidth - 6 - ellipsisWidth).trim() + "..."
        ...
        font.split(text, maxWidth).forEach {
            font.drawInternal(it, x.toFloat(), y.toFloat(), colour, matrix, shadow)
        }
    }
    ...
    protected open inner class LMButton(
        ...
        override fun render(matrixStack: MatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            if (hasIcon) {
                mc.textureManager.bind(textureResLoc)
                RenderSystem.color3f(1F, 1F, 1F)
                blit(
                    matrixStack,
                    x,
