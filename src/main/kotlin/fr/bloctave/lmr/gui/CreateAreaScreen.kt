...
class CreateAreaScreen(
	...
	override fun init() {
		...
		nameInputField = TextFieldWidget(font, guiLeft + 4, guiTop + 15, 110, 9, StringTextComponent("")).apply {
			setBordered(false)
			changeFocus(true)
			this@CreateAreaScreen.setInitialFocus(this)
		}
		children += nameInputField
		extendCheckBox = addButton(object : ToggleButton(4, 26, 118, 0, I18n.get("gui.lmr.create.checkbox")) {
		...
		addButton(object : Button(
			...
			43,
			12,
			TranslationTextComponent("gui.lmr.create.confirm"),
			{ complete() }) {
	...
	override fun render(matrixStack: MatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float) {
		...
		drawLangString(matrixStack, "gui.lmr.create.area", 5 + guiLeft, 5 + guiTop)
	}
	...
	private fun complete() {
		if (sentCreateMessage)
			return
		val areaName = nameInputField.value.trim()
		if (areaName.isNotEmpty()) {
			val area = Area(areaName, dim, pos1, pos2)

			if (extendCheckBox.isOn)
				area.extendToMinMaxY(minecraft!!.level!!)
