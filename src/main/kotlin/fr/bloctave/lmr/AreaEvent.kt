 ...
 * Handlers that cancel this should have their own way of notifying the user if necessary.
 */
class AreaClaimApprovalEvent(val request: Request, area: Area, val player: ServerPlayerEntity) : AreaEvent(area) {
	override fun isCancelable(): Boolean = true
}
 ...
 * Handlers that cancel this should have their own way of notifying the user if necessary.
 */
class AreaClaimDisapprovalEvent(val request: Request, area: Area, val player: ServerPlayerEntity) : AreaEvent(area) {
	override fun isCancelable(): Boolean = true
}
