...
import fr.bloctave.landmanager.data.requests.Request
 ...
 */
class AreaDeletedEvent(area: Area) : fr.bloctave.landmanager.AreaEvent(area)
 ...
 */
class AreaCreationEvent(area: Area) : fr.bloctave.landmanager.AreaEvent(area) {
 ...
 * Handlers that cancel this should have their own way of notifying the user if necessary.
 */
class AreaClaimEvent(val player: PlayerEntity, area: Area) : fr.bloctave.landmanager.AreaEvent(area) {
	override fun isCancelable(): Boolean = true
}
 ...
 * Handlers that cancel this should have their own way of notifying the user if necessary.
 */
class AreaClaimApprovalEvent(val request: Request, area: Area, val sender: CommandSource) : fr.bloctave.landmanager.AreaEvent(area) {
	override fun isCancelable(): Boolean = true
}
 ...
 * Handlers that cancel this should have their own way of notifying the user if necessary.
 */
class AreaClaimDisapprovalEvent(val request: Request, area: Area, val sender: CommandSource) : fr.bloctave.landmanager.AreaEvent(area) {
	override fun isCancelable(): Boolean = true
}
