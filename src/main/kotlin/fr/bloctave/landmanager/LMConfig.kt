...
object LMConfig {
	...
	private val COMMON: LMCommonConfig
	...
	init {
		...
		ForgeConfigSpec.Builder().configure { LMCommonConfig(it) }.apply {
			COMMON = left
			COMMON_SPEC = right
		}
	...
	fun bake() {
		...
		// Global
		/*canPassiveSpawn = COMMON.canPassiveSpawn.get()
		canHostileSpawn = COMMON.canHostileSpawn.get()
		canExplosionsDestroyBlocks = COMMON.canExplosionsDestroyBlocks.get()
		canPlayersInteract = COMMON.canPlayersInteract.get()
		canPlayersBreakBlocks = COMMON.canPlayersBreakBlocks.get()
		canPlayersPlaceBlocks = COMMON.canPlayersPlaceBlocks.get()*/
		// Permissions
		claimRequest = COMMON.claimRequest.get()

		tool = COMMON.tool.get()
		rename = COMMON.rename.get()
	}
...
class LMCommonConfig(builder: ForgeConfigSpec.Builder) {
